...
import {
  pickRichting,
  buildAdviceParagraphs,
  buildJudgmentAdvice,
  buildFirstStep,
} from "@/lib/scan/advice";
...
function ScanPage() {
      ...
      <div className="flex min-h-[100dvh] flex-col md:flex-row">
        <div className="order-2 flex w-full flex-col justify-center bg-cream px-5 py-16 md:order-1 md:w-3/5 md:px-16 md:py-20">
          <div className="mx-auto w-full max-w-xl">
            ...
            <p className="mt-7 text-lg leading-relaxed text-ink/70">
              Terugkerend werk hoort bij ieder bedrijf. Wanneer mensen steeds dezelfde informatie
              verzamelen, berekenen of overnemen, is dat vaak de moeite van bekijken waard.
            </p>
            <p className="mt-5 text-lg leading-relaxed text-ink/70">
              Kies één proces en krijg in ± 4 minuten een eerste, concrete diagnose: hoeveel tijd
              erin zit, hoe groot de kans is, en waar je zelf niets aan zou veranderen.
            </p>
            <button
              type="button"
        ...
        <div className="order-1 flex w-full flex-col justify-center bg-cream px-5 py-16 md:order-2 md:w-2/5 md:px-10 md:py-20">
          <div className="mx-auto flex w-full max-w-sm flex-col justify-center rounded-2xl bg-ink-hero p-8 text-cream md:h-[522px] md:p-10">
            ...
            <p className="mt-8 text-2xl leading-snug text-cream md:text-3xl">
              Aanvragen &amp; calculaties
            </p>
  ...
  return (
    <div className="flex flex-col md:flex-row">
      <div className="order-2 flex w-full flex-col justify-center bg-cream px-5 py-16 md:order-1 md:w-3/5 md:px-12 md:py-20">
        {step ? (
          <div className="mx-auto w-full max-w-xl">
            ...
            <div key={step.id} className="fade-up mt-10">
              ...
              {step.sub ? (
                <p className="mt-3 text-base leading-relaxed text-ink/60">{step.sub}</p>
              ) : null}
              ...
              <div className="mt-8">
                ...
                {step.kind === "choice" && step.id !== "process" ? (
                  <ChoiceTiles
                    ...
                    onToggle={(value) =>
                      toggleMulti(step.id as "timeSinks" | "sources" | "impact", value, step.max)
                    }
                  />
                ) : null}
      ...
      <div className="order-1 w-full px-5 py-16 md:order-2 md:flex md:w-2/5 md:flex-col md:justify-center md:px-10 md:py-20">
        <div className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center rounded-2xl bg-ink-hero p-8 text-cream md:p-10">
          <ScanLivePanel answers={answers} stepIndex={stepIndex} />
        </div>
      </div>
    </div>
  );
