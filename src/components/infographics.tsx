...
export function FocusVsChaos({ className }: { className?: string }) {
  const chaos = [
    [250, 30],
    [310, 70],
    [232, 120],
  ...
  return (
    <svg viewBox="0 0 400 470" className={className} role="img" aria-label="Vergelijking: geautomatiseerd proces versus handmatig proces met onderbrekingen">
      ...
      <line x1="70" y1="40" x2="70" y2="420" stroke={forest} strokeWidth="2.5" />
      {[40, 166, 293, 420].map((y) => (
        <circle key={y} cx="70" cy={y} r="8" fill={copper} />
      ))}

      <path d={path} fill="none" stroke={ink} strokeOpacity="0.45" strokeWidth="2" strokeLinejoin="round" />
      {chaos.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r={i % 3 === 0 ? 7 : 5} fill={i % 3 === 0 ? ink : sage} />
      ))}
      {noise.map(([x, y], i) => (
        <circle key={`n${i}`} cx={x} cy={y} r="5" fill={sage} opacity="0.75" />
      ))}

      <g fontSize="11" fill={ink} opacity="0.7">
        <line x1="24" y1="450" x2="52" y2="450" stroke={forest} strokeWidth="2.5" />
