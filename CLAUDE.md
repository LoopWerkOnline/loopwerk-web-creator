...
- **Blogs** (`/blog`, in het menu "Blogs"; eigen sectie, geen blok op andere pagina's): sectorgericht en functioneel, iets wat de lezer vandaag kan toepassen (bv. "5 ChatGPT-prompts om sneller nieuwe klanten te vinden als installatiebedrijf"). Zo min mogelijk over Loopwerk (hooguit subtiel in de afsluiter). Nuchter, persoonlijk Nederlands, niet fancy. Opmaak rustig en verticaal: fotohero zoals overal (`PhotoHero`), één tekstkolom, lopende tekst zonder opsommingen, alleen blokken waar ze functioneel zijn (prompt-blok met kopieerknop). Overzicht = kaartjes. Geen cijfers zonder bron. Reeksen: AI concreet, Achter de aanvraag, Eerlijk advies; 2 per maand. Ideeën en kalender: SharePoint `02 - Marketing/Blogideeën Loopwerk.docx`.
...
- TanStack Start + React + Tailwind (oorspronkelijk gebouwd met Lovable). Content staat grotendeels in `src/lib/content.ts`.
- **Hosting en deploy: alleen Vercel.** Lovable wordt niet meer gebruikt. Repo `LoopWerkOnline/loopwerk-web-creator`; elke push naar `main` deployt automatisch naar Vercel-project `loopwerk-web-creator-45vx` (team `loopwerk1`). De build staat expliciet op Nitro-preset `vercel` (`vite.config.ts`). Vóór push: `npx tsc --noEmit`, eslint op gewijzigde bestanden en `npx vite build`; daarna de commit-status ("Vercel – loopwerk-web-creator-45vx") controleren.
- Supabase: `contact_requests` en `scan_leads` (insert-only voor anon, RLS aan). Migraties in `supabase/migrations/`; nieuwe tabellen altijd met expliciete `GRANT`. Het Supabase-project is ooit via Lovable Cloud aangemaakt; migraties voer je uit in het Supabase-dashboard (SQL editor) of met de CLI, niet via Lovable. De server leest `SUPABASE_URL`/`SUPABASE_PUBLISHABLE_KEY` en valt terug op de publieke `VITE_SUPABASE_*` uit `.env`.
...
- Env-variabelen (in Vercel → Settings → Environment Variables, nooit in code): `HUBSPOT_PORTAL_ID`, `HUBSPOT_FORM_CONTACT`, `HUBSPOT_FORM_SCAN`, `HUBSPOT_PRIVATE_APP_TOKEN` (deals), `VITE_HUBSPOT_PORTAL_ID`, `VITE_HUBSPOT_REGION`, `VITE_GA4_ID`, `RESEND_API_KEY`, `LEAD_NOTIFY_TO`, `LEAD_NOTIFY_FROM`. Ontbreken ze, dan wordt die stap overgeslagen.
- Geen API-keys of tokens in de code; gebruik environment variables / Supabase secrets.
- Geen force-push of history rewrites op `main`.
